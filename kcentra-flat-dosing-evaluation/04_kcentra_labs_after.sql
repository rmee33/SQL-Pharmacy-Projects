...
Purpose: Extract candidate lab results after first Kcentra administration (post-dose monitoring).

Inputs (logical):
- ehr.lab_result, ehr.event (optional for Vitamin K), pharmacy.kcentra_admin

Outputs:
- Row-level lab result set for encounters after first administration
...
SELECT * FROM labs_post
UNION ALL
SELECT * FROM vitk_post;
